...
Deno.test('Units.inHgToHpa', async (t) => {
  ...
  await t.step('1 inHg → 33.8639 hPa (definition)', () => assertAlmostEquals(Units.inHgToHpa(1), 33.8639, 0.001));
  await t.step('zero is zero', () => assertEquals(Units.inHgToHpa(0), 0));
  await t.step(
    'typical high pressure: 30.5 inHg',
    () => assertAlmostEquals(Units.inHgToHpa(30.5), 30.5 * 33.8639, 0.01),
  );
});

Deno.test('Units.mphToMs', async (t) => {
  await t.step('1 mph = 0.44704 m/s (exact definition)', () => assertAlmostEquals(Units.mphToMs(1), 0.44704, 0.00001));
  await t.step('11.185 mph → ~5.0 m/s', () => assertAlmostEquals(Units.mphToMs(11.185), 5.0, 0.005));
  await t.step('17.895 mph → ~8.0 m/s', () => assertAlmostEquals(Units.mphToMs(17.895), 8.0, 0.005));
  await t.step('zero is zero', () => assertEquals(Units.mphToMs(0), 0));
  await t.step('60 mph = 26.8224 m/s', () => assertAlmostEquals(Units.mphToMs(60), 26.8224, 0.001));
});
...
Deno.test('Units.inToMm', async (t) => {
  ...
  await t.step('0.157 in → ~3.99 mm', () => assertAlmostEquals(Units.inToMm(0.157), 3.9878, 0.001));
  await t.step('12 in = 304.8 mm (one foot)', () => assertAlmostEquals(Units.inToMm(12), 304.8, 0.001));
});

Deno.test('Units.inHrToMmHr', async (t) => {
  await t.step('1 in/hr = 25.4 mm/hr (same factor as inToMm)', () => assertEquals(Units.inHrToMmHr(1), 25.4));
  await t.step('0.039370 in/hr → ~1.0 mm/hr', () => assertAlmostEquals(Units.inHrToMmHr(0.039370), 1.0, 0.001));
